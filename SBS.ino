int trigpin1 = 8; // Defines Tirg pins of the Ultrasonic 
Sensor 1  
int echopin1 = 7; // Defines Echo pins of the Ultrasonic Sensor 1   
int trigpin2 = 10; // Defines Tirg pins of the Ultrasonic 
Sensor 2  
int echopin2 = 9; // Defines Echo pins of the Ultrasonic Sensor 2   
int BUZZpin = 3; // Defines BUZZER pins 
int  
Vmotor = 2; // Defines Vibrating motor pins   
void 
setup()   
{   
Serial.begin (9600);   pinMode(trigpin1,OUTPUT); // Sets the 
trigPin1 as an Output   pinMode(echopin1,INPUT);  //  
...
loop()   
{   
// measure distane using ultrasonic sensor 1 and Print sensor deta in serial 
monitor     
...
// delay(500); // delay for clearly shown in serial monitor   
//if any sensor senses distance less than 80 cm then the buzzer generates sound and 
the motor starts vibrating.     
...
//if any sensor senses distance less than 40 cm then the buzzer generates sound and 
the motor starts vibrating faster     
{   
digitalWrite(BUZZpin,HIGH);       
delay(50);   
digitalWrite(BUZZpin,LOW);       
...
delay(50);   
// else the buzzer generates sound after every 3 seconds for indicating the device 
is on              
{   
      digitalWrite(BUZZpin,LOW);        
digitalWrite(Vmotor,LOW);   
